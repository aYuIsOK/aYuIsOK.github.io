...
let peoplecanvasEll = document.getElementById("peoplecanvas");

let ctx = peoplecanvasEll ? peoplecanvasEll.getContext("2d") : undefined,
    stage = {
        width: 0,
        height: 0,
    },
...
function init() {
    if (!peoplecanvasEll) return;
    createPeeps(), resize(), gsap.ticker.add(render), window.addEventListener("resize", resize);
}

document.addEventListener("pjax:success", e => {
    peoplecanvasEll = document.getElementById("peoplecanvas");
    if (peoplecanvasEll) {
        (ctx = peoplecanvasEll ? peoplecanvasEll.getContext("2d") : undefined), window.removeEventListener("resize", resize);
...
function resize() {
    if (peoplecanvasEll && peoplecanvasEll.clientWidth != 0) {
        (stage.width = peoplecanvasEll.clientWidth),
            (stage.height = peoplecanvasEll.clientHeight),
            (peoplecanvasEll.width = stage.width * devicePixelRatio),
            (peoplecanvasEll.height = stage.height * devicePixelRatio),
...
function render() {
    if (!peoplecanvasEll) return;
    (peoplecanvasEll.width = peoplecanvasEll.width),
        ctx.save(),
